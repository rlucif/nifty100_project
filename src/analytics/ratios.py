...
def calculate_cfo_quality_score(operating_activity, net_profit):
   if net_profit == 0:
      logger.debug('CFO Quality Score calculation skipped because net profit is zero')
      return None
...
def calculate_average_cfo_quality_score(cfo_quality_scores):
   ...
   if not valid_scores:
      logger.debug('Average CFO Quality Score calculation skipped because no valid scores are available')
      return None
...
def calculate_capex_intensity(investing_activity, sales):
   if sales == 0:
      logger.debug('CapEx Intensity calculation skipped because sales is zero')
      return None
...
def calculate_fcf_conversion(free_cash_flow, operating_profit):
   if operating_profit == 0:
      logger.debug('FCF Conversion calculation skipped because operating profit is zero')
      return None
